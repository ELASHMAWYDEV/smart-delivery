		...
		for (let message of messages) {
			...
			if (
				(langList.length != 0 &&
					langList.filter(
						(item) =>
							item.includes("arabic") || item.includes("farsi") || item.includes("pashto") || item.includes("urdu")
					).length != 0) ||
				body == "خروج" ||
				body == "١" ||
				body == "٢" ||
					...
					if (userQuestion.get(author.split("@")[0])) {
						...
						switch (questionKey) {
							...
							case "LOCATION_INFO":
								...
								if (!url) {
									//Check if sent "1" || "Hello" || "Exit" || "خروج" --> start chat again
									let { bestMatch } = stringSimilarity.findBestMatch(body, ["exit", "hello", "1", "١"]);
