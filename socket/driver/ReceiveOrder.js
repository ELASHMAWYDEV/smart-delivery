...
module.exports = (io, socket) => {
	socket.on("ReceiveOrder", async ({ branchId, driverId, token, language }) => {
		/*
		...
		/******************************************************/
		try {
			console.log(`ReceiveOrder event was called by driver: ${driverId}, branch: ${branchId}`);
			/********************************************************/
			//Developement errors
			if (!branchId)
			...
			//Update the orders
			const updateOrdersResult = await receiveOrder({ token, language, branchId });
			...
			socket.emit("ReceiveOrder", {
				status: true,
				isAuthorize: true,
				message: `Orders ${ordersIds.map((order) => "#" + order)} have been received successfully`,
			});
