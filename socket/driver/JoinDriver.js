...
//Helpers
const { checkForOrderRequest, checkDriverActiveDialogues } = require("../../helpers");
...
module.exports = (io, socket) => {
  socket.on("JoinDriver", async ({ driverId, token, language, firebaseToken, deviceType = 2 }) => {
    ...
    try {
      ...
      let busyActiveOrders = busyOrders.filter((order) => [3, 4].includes(order.master.statusId));
      ...
      if (busyCreatedOrders.length != 0) {
        checkForOrderRequest({ socket, driverId, language }); //Send him orders that he have not seen
      }

      // Check if there are any persistent dialogues active on mobile & can be removed
      checkDriverActiveDialogues({ driverId });
      /***************************************************/
    } catch (e) {
